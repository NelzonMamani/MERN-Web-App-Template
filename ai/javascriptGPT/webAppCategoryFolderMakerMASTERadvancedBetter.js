...
function cleanCategoryNames(folderName, fileName) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    //5. create a subdirectory folder called frontend
    const frontendfolder = path.join(subdirectory, 'frontend');
    if (!fs.existsSync(frontendfolder)) {
        fs.mkdirSync(frontendfolder);
    }

    //create .env file
    createDotEnv(backendfolder);
    //create .gitignore file
    createDotIgnore(backendfolder);
